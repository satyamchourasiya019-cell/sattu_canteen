...
import { confirmScan, currentEmployeeSerial, getScanContext } from '../../services/scanService';
import { friendlyMessage } from '../../utils/errors';
import { formatCurrency, formatDisplayDate, formatTime12h } from '../../utils/format';
import type { ScanContext } from '../../types';

type Phase = 'loading' | 'confirm' | 'done' | 'error';

/**
 * QR destination page: /qr/breakfastSnacks and /qr/lunchDinner.
 * The SERVER decides which meal is active right now (phone clocks are not
 * trusted). A duplicate scan offers extra items instead of double-charging.
 */
export default function ScanPage(): JSX.Element {
  const qrType = window.location.pathname.endsWith('lunchDinner') ? 'lunchDinner' : 'breakfastSnacks';
  ...
  const loadContext = useCallback(async () => {
    ...
    setErrorCode(null);
    try {
      const c = await getScanContext(qrType);
      setCtx(c);
      setSelected([]);
      setPhase('confirm');
    } catch (err) {
  ...
  useEffect(() => {
    if (!currentEmployeeSerial()) {
      setErrorCode('NO_SESSION');
      setErrorMsg('Please register first — one time only.');
      setPhase('error');
      return;
    }
    void loadContext();
  ...
  if (phase === 'error') {
    const isNoSession = errorCode === 'NO_SESSION' || errorCode === 'UNAUTHENTICATED';
    return (
      <div className="order-wrap">
        <div className="order-card error-card">
          <div className="error-icon" aria-hidden>!</div>
          <h1>{isNoSession ? 'Register First' : 'Cannot Record Right Now'}</h1>
          <p>{errorMsg}</p>
          <div className="btn-row">
            {isNoSession ? (
              <Link className="btn-primary" to="/user-ordering">Go to Registration</Link>
            ) : (
              <button type="button" className="btn-primary" onClick={() => void loadContext()}>Try Again</button>
            )}
    ...
    return (
      <div className="order-wrap">
        <div className="order-card success-card">
          ...
          <div className="btn-row">
            <button type="button" className="btn-primary" onClick={() => void loadContext()}>Scan Again</button>
            <Link className="btn-ghost" to="/user-ordering">Home</Link>
          </div>
        </div>
      </div>
